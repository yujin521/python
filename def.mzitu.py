...
class mzitu():
  ...
  def all_url(self, url):
      ...
      all_a = BeautifulSoup(html.text, 'lxml').find('div', class_='pic').find_all('a')
      for a in all_a:
          title = a.get_text()
  ...
  def img(self, page_url):  ##这个函数处理图片页面地址获得图片的实际地址
      ...
      img_url = BeautifulSoup(img_html.text, 'lxml').find('div', class_='main-content').find('img')['src']
      self.save(img_url)

  def save(self, img_url):  ##这个函数保存图片
      name = img_url[-9:-4].replace('/','_')
  ...
  Mzitu = mzitu()  ##实例化
  Mzitu.all_url('http://www.mmjpg.com/hot/')  ##给函数all_url传入参数  你可以当作启动爬虫（就是入口）